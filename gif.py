...
saveFileNameTrap = '1000,6,5 Trap at 0.png'

#Do you want to plot the energy of the system through the simulation?
computeKineticEnergy = True
saveFileNameEnergy = '1000,6,5 Kinetic Energy at 0.png'
...
saveFileNameCentralDensity = '1000,6,5 Density at 0.png'
...
saveFileNameTemperature = '1000,6,5 Temperature at 0.png'
...
computeGifs = True
saveFileNameGif = '1000,6,5 Evolution at 0.gif'
velocityBinsNumber = 40
#----------------------------------------------
#Data unpacking
#----------------------------------------------
...
#Delete al particles in speed and positions that dont correspond to the radius we want
positionsMatrix = list(csv.reader(open(positionsFile), delimiter=','))
speedsMatrix = list(csv.reader(open(speedsFile), delimiter=','))
i = 0
while i < len(positionsMatrix):
    if positionsMatrix[i][0] != str(radiusIndexInterest):
        del positionsMatrix[i]
        del speedsMatrix[i]
    else:
        i += 1
...
for row in positionsMatrix:
    ...
    for i in range(len(row)):
        row[i] = float(row[i])
if(radiusIndexInterest != 0):
    chargeMacro = 8 * radiusIndexInterest * chargeMacro
    massMacro = 8 * radiusIndexInterest * massMacro
    weightMacro = 8 * radiusIndexInterest * weightMacro
...
if plotTrapPotential:
    ...
    for i in range(len(zAxis)):
        if zAxis[i] >= zMin and zAxis[i] <= zMax:
            ...
            shortTrap.append(trapPotentialMatrix[radiusIndexInterest][i])
    ...
    anAxis1.plot(zAxis, trapPotentialMatrix[radiusIndexInterest]) 
    ...
    expected = (math.sqrt(2 * coefs[2] * chargeSingle / massSingle)) / (math.pi * 2)
...
if computeKineticEnergy:
    ...
    figEnergy, (axOne) = matplotlib.pyplot.subplots(1, sharex = True)

    axOne.set_title('Kinetic Energy')
    axOne.set_ylabel('KE (V)')
    axOne.plot(timesMatrix[0], totalKinetic)
    axOne.ticklabel_format(axis='both', style='sci', scilimits=(0,0), useOffset=False, useLocale=False, useMathText=True)
    axOne.set_xlabel('time (s)')
...
if computeRhoCentre:
    ...
    if radiusIndexInterest == 0:
        ...
        volume = math.pi * hz * hr * 2 * radiusIndexInterest * hr
...
if computeGifs:
    ...
    matplotlib.pyplot.figtext(0.5, 0.01, f'Simulation running {len(positionsMatrix)} macro-particles at a radius index of {radiusIndexInterest}. Macro-particles with mass and charge of {massMacro} kg and {chargeMacro} C respectively.', wrap=True, horizontalalignment='center')
